        ...
        try {
            ...
            httpClient.newCall(request).enqueue(new okhttp3.Callback() {
                ...
                public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                    if (response.isSuccessful()) {
                        Log.d(TAG, "Статус лайка успешно обновлен на сервере: recipeId=" + recipe.getId() + ", isLiked=" + isLiked);
                    } else {
                        Log.e(TAG, "Ошибка при обновлении статуса лайка: " + response.code() + " " + response.message());
                    }
                }
            });
        } catch (Exception e) {
